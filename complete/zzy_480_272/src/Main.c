...
extern char Image$$ZI$$Base[];
	...
	extern unsigned char cai[];
	extern unsigned char niu[];
...
//-----add bgn-------------------
	...
	{
		get_pix(&pix_x,&pix_y);
		offset_x=(pix_x-480)/2;
		offset_y=(pix_y-272)/2;
		...
		for(ii=0;ii<16;ii++)
		{
			if(ii<8) 
				...
				iii=(ii-7)*10;	
			Paint_Bmp(0+offset_x, 0+offset_y, 480, 272,cai);
			...
			Delay(500);
			Paint_Bmp(0+offset_x, 0+offset_y, 480, 272,niu);
			//Glib_FilledRectangle(200+offset_x,40+offset_y,240+offset_x,80+offset_y,0x0000);
			//Paint_Bmp(205-10*ii+offset_x,iii+offset_y, 14, 14, ball_014014);
			Delay(500);
			Paint_Bmp(0+offset_x, 0+offset_y, 480, 272,cai);
